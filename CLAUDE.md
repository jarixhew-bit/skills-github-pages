...
4. **建东西必配自检**（2026-08-01 用户要求：「以后的都要自检」）：新建长期存在的
   ...
   一次死掉过半时先抓一张新图来探，新图也打不开就判定是这台机器被 Google 挡了、
   整轮罢工不动任何文件，否则一次误判会把整本好图换掉。补图＝机器的事，
   挑图「哪张好看」仍然是人的事）。
   **手册照片不要菜单照**（2026-09-19 用户要求：「图片我要求店＋食物的，不要出现
   菜单的」）：抓图器从 Google 的「Food & drink」分类取图，之后 `photo-filter` 再用 OCR
   数一次字（满屏文字＋一排价格＝菜单），命中的排到该店候选最后而不是删掉。
   它的自检 `check-photo-filter` **只能在 CI 跑**（要 tesseract 与 pillow），
   会自己画一张菜单当阳性样本——真实候选图里一张菜单都没有，拿真图测等于没测。
   **走完的行程不再体检也不再补图**（2026-09-19 用户要求：「行程过了的就不用再跑了，
   只跑还没到的」）——哪几趟还没走完，正本是 `tools/lib/trips.py` 的日期表，
   体检与补图都从那里读；**新手册建好要在那张表加一行**（没登记会被当成常青页照常跑，
   自检会点名漏登记的手册）。
