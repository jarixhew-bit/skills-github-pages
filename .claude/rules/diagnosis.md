...
- [2026-07-13／2026-08-03][雲端] 情境：想確認一個改動「真的上線了／真的提交進去了」。
  教訓：沙盒**連不到** `github.io`、`*.workers.dev`（代理策略性 403，別重試、也別懷疑對方掛了），
  只能間接驗：(1) `git show origin/main:路徑` 從 **remote** 讀回——工作區正確不代表 commit 進去了
  （混用 `git rm` 與 Write/Edit 時最容易漏，commit 前跑 `git diff --cached --stat` 或直接 `git add -A`）；
  (2) 查 CI 是否 success（`actions_list` 回傳動輒 40 萬字元，落檔後用 jq 提取，別直接讀）。
  線上那一層已由 `tools/check-live.py` ＋ `live-check.yml` 每天自動驗（2026-08-11 建），
  不必再手動查 deployments API。**CI 綠了不等於線上驗過**，真實端到端仍只能請使用者用一次。
  另：CI 抓回來的資料要給 Claude 用時，讓 CI **commit 回觸發分支**——artifact 要登入才能下載、
  日誌尾部又常被收尾步驟淹掉，沙盒兩條都讀不到（2026-08-11 為此猜了兩輪才換路）。
  來源：清空公司帳本時帳本 JSON 沒 add，照片全刪、記錄全在，使用者月度導出才發現；
  鮨酒場更新時 Pages 部署事件被整個丟掉。
...
- [2026-08-05／2026-08-11][皆是] 情境：写检查工具、或改检查工具的等待方式，跑完报「通过」。
  ...
  (4) `node xxx | tee out.txt` 的退出码取自 `tee`，脚本崩溃了那一步照样显示成功；而仓库
  `package.json` 若是 `type: module`，`.js` 里的 `require` 会直接失效——两件叠在一起，
  CI 绿了三分钟其实什么都没跑，是看「耗时 0 秒」才发现的。**管道一律加 `set -o pipefail`。**
  (5) 「双击生出两条一样的记录」这类闸门自测通过、真机仍重复：双击是两个各自跑到底的
  事件，不是竞态——加锁解锁若在同一个执行栈里做完，第二次点击派发时锁早解开了，等于没挡；
  要用 `setTimeout` 延后解锁。测法同样有坑：必须 `el.click(); el.click();` 连打两下，
  `page.evaluate` 连调两次是顺序调用、Playwright 的 `page.click()` 会等元素可操作，都测不出来。
  附带一坑：往 `git cat-file --batch` 的 stdin 一次性写几百 KB 会死锁（管道满了两边互等），
  要用独立线程喂；当时误判成「正则回溯」，是因为没先确认「到底卡在哪一步」就动手改代码。
- [2026-08-10][皆是] 情境：想要一个「不可能跟真实数据撞车」的占位符。教训：NUL（0x00）要写
  JS 转义（反斜杠 u0000），别写真实字节——功能全正常、自检全绿，唯一症状是 git/grep 从此把
  文件当二进制，`git diff` 只剩「Binary files differ」，代码审查等于瞎了。踩过两次（第二次
  在管钱的文件上），已由 `check-html.py` 的 `check_control_bytes` 守着，别删。
  通则：**同一类错误出现第二次，就该把它变成脚本，别再靠记性。**
...
- [2026-08-12][皆是] 情境：外部 API 连续多天回同一个错误码，查了好几轮都对不上。教训：
  **先换一台对等主机再送一次——同一个故障在不同主机上的错误码可能天差地别。**
  ...
  `1018 Too many requests`，真正的病是**限流**；同一个问题因此被误诊三次，每次都往
  「报表定义坏了」查。通则：错误码含糊时，**先想办法拿到第二个信息源**（换主机、换端点、
  换客户端），比在单一信息源上反复推理快得多。第二课：**重试次数本身可能是凶手，而
  排查时的手动重跑也算**（当天手动跑七八次、每次烧两个请求，一边查一边加重病情）；限流
  类故障要减少重试、拉长间隔，并把「手动触发也去打那个 API」改成勾选才做。第三课：
  「手动跑得出来、程式跑不出来」正是限流的典型症状（网页走登入 session、不吃额度）。
