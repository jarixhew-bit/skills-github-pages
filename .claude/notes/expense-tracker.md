...
- **老板端**：**首屏**那张「💵 同事手上（你的钱）」卡（2026-09-11 用户要求从设置页
  移过来），**只在「收件账户」被选中时出现**——摆在公司账或他私人账户的首屏上全是
  不相干的数字。同理，`renderOverview()` 里也**只在那一页才 `fetchBossPetty()`**：
  别的账户上这张卡根本不出现，拉了是白打一个请求，而且会跑进别的功能的自检里当噪音
  （加这张卡时就撞红了「发给老板」那一块 15 条）。
  点进去设起点／转钱／调整／**归还**／撤销。
- **一键归还**（2026-09-11）：手上还有钱的人，弹窗列表里多一颗「已还清」，
  预设填他手上全部、可以改成实收的数（还了 4000、剩 5.29 继续留在他名下）。
  底下走的是 **`adjust` 取负**，不另开一种事件类型——多一种类型就多一条算余额的
  分支，多一条分支就多一个地方会算错。备注写「归还现金」，以后翻记录看得出来
  这不是随手调的。
- **镜像事件：钱在人之间移动，两边都要动**（2026-09-11 用户实机指出「每次一点归还，
  全部都回去 Yang 那边」）。这是会让对账**永远**对不上的缺口：转 5000 给 Kuang，
  Kuang +5000 而 Yang 没少 → 合计凭空多 5000；归还同理反向。
  现在 App 送 `mirror:true` 的事件，butler 会自动在 **Yang 名下追加一笔等额反向的
  adjust**（`mirrorOf` 指回主事件），跟公司备用金那边同一套做法。
  **镜像的**：转钱、归还。**不镜像的**：设起点（读一个当下的数）、手工调整（校准，
  钱没有移动）、转给 Yang 自己（会自我抵消）。
  撤销要**整对一起撤**，否则留下「Yang 被扣了钱、同事那笔却撤销了」的半吊子状态；
  `bossPettyLastEvent()` 也回报主事件——摆一句「Yang −5000」给老板看，他会以为按错了。

- **对账（2026-09-11 用户要求「我们都填上了，那个总合要对上老板总账」）**：卡片底部
  多一段——`bossPettyReconcileHtml()`。老板的钱只有两种去向：花掉了（账本里有记录），
  或还在某个人手上。所以

      Yang 手上 ＋ Kuang 手上 ＋ Seryi 手上 ＝ 账户余额

  对不上就是有事情没记（有人拿了钱没记、有人花了没记、某一格起点填错）。
  **差额摆出来给人看，绝不自动抹平**——2026-09-10 那次账乱掉，正是从「加一笔对账
  调整把差额抹掉」开始的，过两天没人记得那个数代表什么。卡上明写「别用『调整』抹平」。
  ⚠️ **有人还没设起点时不给合计**，只说还差谁：把 unset 当 0 加进去会得出一个
  看起来像真的、其实少算一个人的总数（做 false-red 时正好演示了这一点——
  Seryi 没填时合计刚好「对上」）。账户余额用的是跟首屏账户卡同一个算法，不另写一份。

- **人名绝不能拼进 onclick**：`ledEsc()` 只转义 `& < > "`，**不管单引号**，一个单引号
  就把属性撑破。按钮只带行号（`bossPettyPick(i, type)`），名字留在 `bossPettyRows`
  里——这条在「该付同事多少」那边写过一次，加这张卡时又犯了一遍。函数全部 `bossPetty*` 前缀、DOM 也是独立一组
...
### 消费类别：老板定，同事跟着走（2026-09-11）

用户要求「消费项目老板有的都加下去」。他在自己 App 里加过一批（人情往来、出差、
邮费、银行、投资、换汇、Spa…），那些只存在他手机的 localStorage 里，**同事版永远
不会有**——同事记账挑不到对的类别就随便塞一个，月底账本上那笔是什么就没人知道了。

- 老板 App 开机 `pushBossCategories()` 整份推上去（`data/boss-categories.json`）；
  同事版开机 `pullBossCategories()` 拉下来，按 id 补齐＋更新名称图标。
  **老板是唯一权威**，同事版没有设置页、改不了类别，所以不做双向合并。
- ⚠️ **只补不删**：拉到空数组（老板还没同步上来）或没网时，**绝不动本机现有的**——
  清光会让同事当场一个类别都挑不到，比少几个糟得多。自检有这两组对照。
- **删除走名单，不走「列表里没有就删」**：因为上面那道保护，老板删掉一个类别，
  光靠「新的列表里没有它」同事那边不会消失。所以 `pushBossCategories()` 把
  `deletedCategoryIds` 一起送，同事版照那份名单删。
  ⚠️ 只删**本机没有任何记录用到**的：用到了还删，他之前记的账会变成「未分类」，
  那比多一个用不到的类别糟得多（自检有对照组）。
- 服务端内容一样就不写，否则老板每次开 App 都在数据仓库里多一个空 commit。
- 同一份代码里 `pushBossCategories()` 在同事版**不会被调到**：生成时那一行换成
  `pullBossCategories()`（`build-staff-page.py`）。函数本体还在只是因为代码共用。
- ⚠️ `pushBossCategories()` 那一行刻意摆在「启动时拉全员账本」那三行**之外**——
  那一段整块会被生成器剪掉，混在里面这一行会跟着消失。

**踩过的坑：`/boss-expense` 以 `/boss` 开头。** `check-expense-company.mjs` 的
`mountBossRoutes()` 原本用 `startsWith(BOSS_API)` 收请求，于是老板账的请求被算进
「发给老板」那一块的 calls，「只发了一个请求」「action 是 billUpload」当场全红。
改成精确比对。**加新端点时留意会不会跟既有前缀撞车。**
...
## 设置页折成两段（2026-09-13，用户说「太多东西了」）

改之前设置页有 **12 个区块、页面高 5170px**（390 宽的手机上要滑七八屏）。
现在 **2876px**，少掉 44%。分法是「天天用 / 设一次就不用管」：

- **一直看得见**：云端同步、同事投递箱、账户管理、消费类别（只剩标题＋项数）、
  月固定开销、老板与员工（原本的「员工」和「老板 App」两块并成一块，三行都是入口）
- **折起来**（`#more-settings`，默认收着）：公司报账、拍照智能识别、换机准备、数据
- **消费类别的列表**（`#cats-body`，默认收着）：这一块自己就二十几行，占掉旧版快一半
  高度；折起来之后标题底下写「支出 N 项 · 收入 M 项」，看得出里面有东西

两处折叠同一套写法：`applyXxx/toggleXxx/initXxx` ＋ localStorage
（`expenseTracker_moreSettingsOpen`、`expenseTracker_catsOpen`），`init()` 里恢复。
用的是 `hidden` 属性，**元素照样在 DOM 里**——`renderCompanySettings()`、
`#app-build-info` 这些拿 id 找元素的逻辑一个字没改。

### 刻意留在折叠外面的两样（改这块之前先看这段）
1. **`#company-queue-status`（公司账本没送成功的警示）搬到设置页最上面**。它原本长在
   「公司报账」里，而公司报账现在默认是折起来的——警示留在里面等于永远看不见。
2. **版本号 `#app-build-info` 摆在最底下，不收进折叠**。要排查「他手上是不是旧版」的
   时候，正好是最不该让人去翻折叠的时候。

### 自检
`tools/check-expense-company.mjs` 【35】：验「常用那几块一直看得见」（对照组——没有它，
整个设置页被折没了也会显示通过）、「收着的那几块点得开」、展开状态重开 App 还记得、
折起来时警示照样看得见、类别折起来也看得出有几项。
【23】（换机准备）现在要先点开「更多设置」；它前面加了一句断言，展开失败就干脆收场，
不然底下每个 `page.click` 都卡到超时、整份自检直接中断，看不出是哪里红的。

## 管钱的那个人不再单独记一份（2026-09-19，用户拍板，**推翻了 9/11 那版的前提**）

用户原话：「Yang 是管钱的，boss 的账户有多少就是 Yang 的现金」。

### 9/11 那版哪里错了

那版把 Yang 当成跟同事一样的「持有人」：自己有起点、自己算余额，再拿
「大家手上合计」去跟账户余额对，差额摆出来当警报。

**那个对账从第一天起就不可能对上。** Yang 自己记的账从来不进 butler 那本账本
（那本只收 Telegram 和同事版记的），所以 `bossPettyBalances` 算出来的 `spent`
对他永远是 0，余额只增不减。2026-09-19 查证 `data/boss-expenses/2026-09.json`：
17 笔全是 Kuang 的，Yang 一笔都没有。他每记一笔开销，差额就多一笔。

他先后看到的 **273.73、475.00 都是这么来的，不是真的少钱**。我当时让他
「别用调整抹平，先去找那几笔」——找不到的，那几笔根本不存在。

### 现在的算法

    你手上  ＝  账户余额  −  同事手上合计

老板的钱只有三种去向：花掉了（账本里有记录，已经从余额里扣掉）、在同事手上、
剩下的都在管钱的人手上。第三项是**算出来的**，不再单独记一份。

连带取消的东西：
- **镜像事件**（`mirrorOf`）：转 5000 给 Kuang，Kuang 那格 +5000，上面那个减法
  自动让你少 5000。再补一笔镜像就是同一笔钱扣两次。服务端收到 `mirror` 一律忽略，
  App 也不再送。
- **给管钱的人记备用金**：服务端直接挡（`bossPettyRecord` 开头），旧页面绕不过去。
- `data/boss-petty.json` 里 Yang 那两条已清掉（起点 5908.25 ＋ 镜像 3672.39）。

管钱的是谁由服务端给（`petty` 路由回的 `holder`，来自 butler 的
`OWNER_REPORTER_NAME`），**页面不写死**——换人管钱只要改那个 secret。

### 代价：差额那一行没了
同一个数不再算两遍，就没有第二个数可以对。剩下的保护是同事那半边——他们每一格
仍然是「起点 + 转给他的 − 他记的开销」独立算出来的，对不上照样看得出来。
要把「点一下现金跟账面对一次」找回来，得另外做一个**盘点**功能（让他输入实际
数了多少），**那件事没做**。

### 自检
`check-expense-company`【33】：「你手上」那几条断言**锚在那一行上**
（`/你（Yang）手上\s*US\$(-?[\d.]+)/`）——光查页面里有没有这个数会撞上同事自己
那一行，假故障验出来的：把减法拿掉，4500 照样在页面里，因为 Kuang 那格就是 4500。
带对照组「同事多拿 500，你手上就少 500」，否则把「你手上」直接印成账户余额也会全绿。
butler 那边 `tests/boss-expense.test.mjs`【6d】整段重写。

### ⛔ 这张卡**只在 Boss 账户那一页显示**——改过一次，当天被退回（2026-09-19）

用户说「那张卡对我很重要，我给他们钱时要转……怎么不见了，放回本来应该在的位置」。
我判断成「它被账户条件藏起来了」，就改成每个账户都显示。**判断错了。**
他当场退回来：「那个同事手上钱的卡片应该只显示在 boss 账户的那一页，
不是让你全部户口都显示」。

**现在是（也维持）：`curAcc().id === getInboxAccountId()` ＋ 有公司报账密钥。**
他找不到这张卡时，答案是「切到 Boss 账户」，不是把它摊开到每一页。别再改这条。

教训：他说「不见了」不等于「条件写错了」。先问清楚是哪一屏、当时选的是哪个账户，
比直接改设计便宜——这次多花了一个来回。

自检那两条（`★切到别的账户这张卡就收起来` / `没填密钥时也收起来`）都是「该收起来」，
所以**一定要配一条正向的**（`★对照组：切回 Boss 账户、密钥还在，卡片要回得来`），
否则把卡片写成永远隐藏，这一组照样全绿——假故障 G 验出来的。

另外：那一组原本用 `until` 等卡片出现，卡片整个不画时会抛异常、把后面几十条一起
弄哑（假故障时屏幕上一条 ❌ 都没有）。已改成 `waitForTimeout` ＋ 断言 ＋ 画不出来
当场收场。这是这份自检里第二次踩同一个坑（第一次在【23】换机准备）。

## 盘点：拿手上真实的现金去对账面（2026-09-19，用户要的）

同一天把「你手上有多少」改成算出来的之后，对账那一行就没了——同一个数不再算两遍，
就没有第二个数可以对。盘点是把那道保护换个**正确**的方式装回来：
不是同一笔钱算两遍，是拿**人数出来的真实现金**去对账面。

    账面说你手上该有 X   ←  bossExpectedOnHand()（账户余额 − 同事手上）
    你实际数到        Y   ←  人数出来的，系统不可能知道
    差额            Y − X

### ⛔ 差额只记录，绝不给一键抹平的按钮
2026-09-10 那次账乱掉，就是从「加一笔调整把差额抹掉」开始的，过两天没人记得那个数
代表什么。所以这里**连按钮都不给**：要修就去把漏记的那几笔补上，补完再盘一次。
自检里有一条 ★★ 专门守这个（盘点不许生备用金事件、不许往本机账本塞「对账调整」），
假故障验过。

### 怎么算的
- `bossOnHand(rows, acc)` 是**唯一正本**，卡片和盘点弹窗共用。抄成两份的话两边迟早
  算出不同的数——那正是这套账上星期出的那个病。
- 有同事没设起点 → `mine` 回 `null`，**不许拿 0 顶替**（把 unset 当 0 会得出一个
  看起来像真的、其实少算一个人的数）。这时盘点只记「数到多少」，`expected` 送 `null`，
  服务端差额也留空。
- 边打边算差额：按下去才知道差多少的话，他会先自己心算一遍，那这功能就白做了。

### 存哪
butler 的 `data/boss-count.json`，只留最近 60 次。路由 `count` / `countAdd` / `countUndo`，
同事一律 `forbidden`（butler 测试【6g】带老板对照组）。

### 自检
- `check-expense-company`【36】：账面算得对、边打边算、方向讲得对、差额留在卡片上、
  **对得上时不摆警告**（对照组，否则写成永远显示警告也全绿）、算不出账面时送 null、
  负数挡下来。
- `check-staff-page`【31】：老板的现金部位和盘点，同事版一个字都不许出现
  ——**带主 App 对照组**，否则整张卡两边都坏掉也会显示通过。
  （同事版是从主 App 生成的，这段代码会跟着进档案里，服务端挡了，页面这侧也要挡。）

### 写自检时踩到的
- 投递箱账户由 `getInboxAccountId()` 说了算（没设过时它自己猜 `acc_boss`）。
  测试里别自己挑 `data.accounts[0]`——挑错的话账户余额是 0，红的是测试不是产品。
- 新的一组要插在 `await browser.close()` **之前**，插在统计行前面会拿到已关掉的浏览器。

## 出国：每种币一个钱包（2026-09-23，去新加坡当天）

用户开了一个新币的「Singapore trip」账户、把投递箱切过去，转钱给同事那里却还是
显示 US$。查下去比显示更糟，三处都不看币种：

1. **App 这一整套写死美金**（卡片、转钱、盘点共 23 处 `'USD'`）
2. **butler 算备用金不分币种全加在一起**——Kuang 在新加坡花 S$50 会被当成 US$50
   从他的美金余额里扣掉
3. **收件不看币种**：S$50 进了美金账户就变成 US$50

现金是实物，美金和新币是两个钱包，**不能换算着混**（汇率天天变，换算出来的数
跟他手上那叠钞票永远对不上）。

### 现在
- `bossCur()` ＝投递箱账户的币种。卡片、转钱、盘点、撤销全跟着它，请求一律带 `currency`
- 缓存按币种失效：切了投递箱账户就重拉，不拿美金那份缓存当新币显示
- 卡片标题写明币种（「同事手上（你的钱 · SGD）」）——同一张卡两页长得一样，容易看错页
- 输入框标签「金额（SGD · S$）」：美金跟新币只差三成，多打少打一眼看不出
- **收件按币种分流**（`bossAccountForCurrency`）：投递箱是那种币→它；收过同事账的那种币
  账户→它（出国回来切回 Boss 之后，晚到的新币账照样进新加坡那本）；任何那种币的个人
  账户→最新那个；**都没有就先不收**，设置页一直挂着「开一个 XXX 账户就会自动收进去」
- 同事版切币种（`staffSetBossCur`）会马上重拉那种币的钱包

### butler 那边
- 事件带 `currency`，**旧事件没有这个字段的一律当美金**
- 只有美金需要先设起点（上线前的垫付只能靠起点带进来）；其他币种上线后才出现，
  **没设起点就从 0 开始**——不然每出一次国每个人都得先设一次 0 才转得了钱
- 撤销／「上一笔」／盘点全按币种：在新币那一页按撤销，不会撤到后来那笔美金
- 字段名 `amountUsd` 是历史包袱，装的是「这种币的金额」。**不改名**：改名要搬旧数据，
  而旧数据全是美金，不会读错

### 自检
- `check-expense-company`【37】：新币那页全是 S$、请求带 SGD、你手上＝新币账户余额 −
  新币钱包；**对照组**切回美金账户要自己重拉美金钱包（缓存按币种失效）；收件分流
  （新币进新加坡、美金进 Boss、数字原样不换算）；没有日元账户时日元那笔先不收，
  **对照组**开了日元账户下一次收件就自己收进去
- `check-staff-page`【32】：同事切成新币看的是新币钱包；**对照组**切回美金马上重拉
- butler `tests/boss-expense.test.mjs`【6h】【6i】
- 假故障验过：bossCur 写死美金、收件不分流、缓存不看币种、同事切币种后不重拉、
  butler 拿掉按币种过滤、撤销不看币种——六组都红

### 同事版币种跟着老板的投递箱走（2026-09-23，同一天）

用户：「他们那边能不能自动切换币种」→「不要那么麻烦的还要手动，应该全自动的」→
「就不能直接跟着信箱的币种就好，我放新币全部自动跳新币」。

**做法**：主 App 开机时、切投递箱时 `pushBossCurrency()` 把 `bossCur()` 写进 butler
（`settingsPut` → `data/boss-settings.json`，只有老板能写）；同事版开机、切回前台时
`staffPullBossCur()` 读 `settings`，不一样就 `staffApplyBossCur()` 换过去并 toast 一声。
老板那边回 null（没设过／没网）就保持原样。同事手动改过也会被下一次读到的覆盖——
用户要的就是「全部自动跳」。

生成同事版时 `  pushBossCurrency();\n`（开机那行）会被换成 `  staffPullBossCur();\n`。
**切投递箱那一行末尾故意带注释 `// 切投递箱`**，让它跟开机那行字面不同——
不然锚点出现两次，生成直接失败（写的时候踩到）。

**被否决的方案**：先做过「看同事手机时区自动换」，用户中途改成跟投递箱走，已整个撤掉。
代价跟他讲过：留在金边的同事也会一起跳成新币。

**一起修的（跟方向无关、本来就该修）**：
- **币种在记账当下钉在那一笔上**（`tx.bossCur`，`onTxSaved` 里写），不是送出去那一刻才看。
  没网时记的美金，等换成新币之后才送出去，原本会被当成新币——自检【33】★★ 守这条
- 同事版「本月合计」只加现在那种币，另一种写「另有 N 笔是别的币」
- 清单每一笔用自己钉住的币种显示（共用的 `txHtml` 读 `t.bossCur`，主 App 没这字段，不受影响）

**自检**：`check-staff-page`【33】（跟着换／没网不换／钉住币种／合计分币种／切回美金对照组）、
`check-expense-company`【38】（开机写一次、切投递箱马上写、切回美金对照组、没钥匙不写）、
butler【6j】（同事改不动）。假故障验过：切投递箱不写、送出去那一刻才看币种、同事版不跟着换。

### 留在金边的同事不跟着换（2026-09-23，同一天第三次改）

用户：「你美金再怎么录也是新币，还是你有办法把留在金边的同事不切换币种」——投递箱
一切新币，留在金边的人也被换成新币，手动切回美金、下次打开又被换回去。接着问
「去马来西亚呢、印尼呢、越南，你要怎么分辨」。

**现在的规则**（`staffPullBossCur`）：**老板的投递箱决定这趟用什么钱，手机时区决定谁在这趟里**。
- 手机时区是 `STAFF_HOME_ZONES`（`Asia/Phnom_Penh`、`Asia/Bangkok`）→ 一律美金，连老板那边都不用问
- 其他时区 → 跟着老板投递箱的币种
- 手动改过：记下在哪个时区改的，同一个地方照他的；时区一变就回到自动

**看的是时区的名字，不是钟点**。2026-09-23 用 Chromium 141 实测浏览器报的名字：
金边→`Asia/Phnom_Penh`（没有报成曼谷）、胡志明市→`Asia/Saigon`（**旧名**）、
万象→`Asia/Vientiane`、雅加达→`Asia/Jakarta`、吉隆坡→`Asia/Kuala_Lumpur`。
所以雅加达、越南、老挝虽然跟金边同一个钟点，都分得出来。

- 第一版把胡志明市、万象也算在家，越南老挝就永远不会自动换——用户一问就露出来，已拿掉
- **曼谷保守地也算在家**：tzdb 里金边是挂在曼谷底下的别名，Chromium 实测保留金边的名字，
  但 iPhone Safari 没实测过。人在金边被当成出国＝美金被记成新币（用户刚抱怨的那个错），
  比「去泰国要手动点一次」糟，所以宁可泰国不自动
- 要往名单里加越南得写 `Asia/Saigon`；写 `Asia/Ho_Chi_Minh` 对不上（假故障时踩到：
  加了新名自检照样全绿，因为浏览器报的是旧名）

**自检** `check-staff-page`【34】：金边／曼谷时区保持美金、留在金边记的账送美金、
手动改在同地方有效／换地方作废；**对照组**新加坡变新币；马来西亚、印尼、越南、老挝
都认得出不在金边。【33】钉在新加坡时区（不钉的话跑在 UTC+7 的机器上会被当成在金边）。
假故障验过：不看人在哪里全员跟老板、越南（用 `Asia/Saigon`）塞回在家。
