...
### 这张卡不看当下选的是哪个账户（2026-09-19，同一天的第二次修）

原本写成「只在 Boss 账户那一页显示」（`curAcc().id === getInboxAccountId()`）。
用户切到 Yang 账户时**整张卡消失**，他的原话：「那张卡对我很重要，我给他们钱时要转……
怎么不见了，放回本来应该在的位置」。**「转钱给同事」是他每天要按的入口，藏起来＝功能没了。**

现在：只要填过公司报账密钥就一直显示在概览页；数字固定跟着**投递箱那个账户**算
（`getAcc(getInboxAccountId())`），跟当下选哪个账户无关——所以卡里一律用这个 acc，
**不要用 `curAcc()`**。没选投递箱账户时摆一句「设置 → 同事投递箱，选一个账户」，
不默默少一段（少一段看起来就跟坏掉一模一样）。

自检里原本有一条**方向相反**的断言「对照组：切到别的账户这张卡就收起来」——那条把
当时的错设计钉死了。已翻成「★切到别的账户，这张卡照样在」，并补一组真的对照
（没填密钥才收起来），否则「永远显示」也会全绿。
