...
「转钱先抵欠款」规则专用的，规则拿掉后这个函数已经删掉，见下面「转现金不再抵欠款」
那节）：
...
### 转现金不再抵欠款——2026-09-10 用户实机跑完之后拿掉这条规则

**这一节讲的是历史脉络，别照它实现**：2026-09-10 当天先加过一版「转钱给他之前，先拿
这笔钱抵掉他垫付的旧欠款」（转 5000、他垫付过 189，就只记 4811 进代管、欠款标已付），
但用户实机跑完之后明确要求拿掉，原话：

> 「他那边的卡片应该显示 5000−506，我这里应该也是显示 5000 给他，不是 4810.47」
> 「我转个 5000 给他 然后我这边的卡和他那边就是 5000 扣掉这个月他记的就好了不是吗？」

**现在的规则就这么简单**：转现金 = **全额**进代管，`sendBossCashGift()` 不再做任何抵扣
计算——Firestore 的 `boss_cash_gifts.amount` 就是老板输入的原始金额，两条转账腿
（源账户支出 + 代管账户收入）金额也是原始金额，都不带 `offsetTxIds`/`offsetTotal`。
老板欠同事的垫付款**独立存在**（`isUnpaidStaffAdvance()`/`inboxOwedByPerson()` 那一套，
没有变），只有老板在「该付同事」名单里手动按「已付」（`inboxMarkPaid()`）才结清。
两件事各归各的，不再纠缠——`unpaidDebtTxs()`/`pickDebtOffset()` 这两个只服务于旧规则
的纯函数已经删掉，没有别的调用者。
...
**转现金不再抵欠款**（2026-09-10 二度改版拿掉「先抵欠款」规则后）的覆盖挪到【41】：
欠 189.53 转 5000→Firestore 全额 5000/代管拿到整额 5000/欠款仍是 189.53（没被自动结清）/
来源账户减 5000；花 506（`paidFrom:'cash'`）之后代管 4494、本月支出含那 506；对照组
（不欠钱时行为完全一样，证明"不抵扣"不是特例分支）；撤销抵扣
（`offsetGiftGroups()`/`undoBossDebtOffset()`：造一笔旧规则已经抵过的资料，撤销后两条
腿改回全额、代管余额加回、欠款恢复、云端走"删旧建新"且字段合规，可重复点、没有可撤销
的会提示）。自检脚本里的编号以文件里 `console.log`/`ok()` 实际标注为准，这里只是帮助
定位改了哪几块。
## 同事「一键归还」（2026-09-10）
...
自检：`check-expense-company.mjs`【44】用用户真实数字做端到端核对（转账不再抵欠款之后
已经改写，见下面「转现金不再抵欠款」那节）：欠 189.53 转 5000（全额）→ 他手上剩
4776、用现金花掉 224、来源账户净变化 −5189.53、本月支出 413.53、欠款仍是 189.53
（没有被转账自动结清），外加**两边口径一致**那条（老板端算的「他还剩」必须等于同事端
算的「手上还剩」——以后谁把任何一边改歪，这条直接红）、`own` 不走 3 条腿的对照组、以及
手动按「已付」之后重复收件不让已结清垫付复活的回归（新规则下欠款不再靠转账自动结清，
这条回归改用手动标已付触发同一条 `mergeInboxTx()` 路径）。`check-staff-page.mjs` 里旧的
「超支」相关断言按新语义改写（不是删掉）。做过 false-red：把 `mergeInboxTx()` 的保留
条件改成恒假，立刻红 3 条。
...
是「转钱先抵欠款」规则存在的那段时间踩的，规则本身已经拿掉（见上面「转现金不再抵欠款」
