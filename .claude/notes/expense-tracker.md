...
同一条精神。**⚠️ `amount` 字段例外**（2026-09-10 加的"转钱先抵欠款"规则，见下面「代管
账户」那节）：写进 Firestore 的不是用户输入的原始金额，是抵掉垫付欠款之后的净额——这
不是汇率换算也不是猜币种（币种、汇率这两样从头到尾没被算过），是刻意的记账规则调整，
同事那边「手上现金」卡同步的正是这份净额，两边要保持一致。
...
`isUnpaidStaffAdvance(t)` 是唯一正本判断——`inboxOwedByPerson()`、`unpaidDebtTxs()`、
`inboxMarkPaid()` 三处都调它，保证「欠多少」这个数字处处一致：
...
### 转钱给他之前，先拿这笔钱抵掉他垫付的旧欠款（2026-09-10 用户追加要求）

场景：转 5000 给 Kuang 之前，Kuang 已经自己垫付过 189（那笔在他报账时已经记成真支出、
已经扣过账户余额了）。用户要的结果：**欠款归零，他手上的代管现金是 4811，不是 5000**
——因为 189 已经用于结清旧账，不该再算进他手上"待处理"的代管现金；也**不能因为这次
转账又把那笔垫付重新记一次支出**（钱在他报账时就已经算作花掉一次了）。

`sendBossCashGift()` 新流程：
1. `unpaidDebtTxs(person, currency)`——这个人这个币种还没结清的垫付，按日期从旧到新
   排序（跟 `isUnpaidStaffAdvance` 用同一条判断，保证跟"该付同事"名单一致）。
2. `pickDebtOffset(unpaid, amount)`——从最旧的开始逐笔累加，**加到「再加下一笔就会
   超过转账金额」为止就整段停手**（不拆单笔、不跳过一笔大的去凑后面更小的，用户明确
   要求「不拆分单笔」）。返回选中的记录和合计 `offsetTotal`。
3. 只为 `netAmount = amount - offsetTotal` 记转账两条腿（源账户支出 + 代管账户收入，
   都 `xfer:true`）；`offsetTotal` 那部分**不记任何新交易**——原因见上面的重复计账
   算式，那笔钱在报账时已经算过一次了。
4. 被抵掉的那几笔垫付标记 `fromStaff.paidAt`（就是「标记已付」那套写法，不新记支出）。
5. Firestore 的 `boss_cash_gifts` 文档 `amount` 字段存**抵扣后的净额**（不是原始输入）
   ——同事那边「手上现金」卡同步的就是这份数据，两边要保持同一个数字：净额才是他手上
   真正待处理的代管现金。有抵扣时文档额外带 `offsetTxIds`（选中的欠款 id 数组）和
   `offsetTotal`，供撤回时还原。

核对（转 5000、欠 189）：来源账户净变化 = 当初记垫付时扣的 189（早就扣过了，这次没有
再扣）+ 这次 4811 = 5000，跟实际交出去的现金一致；支出统计不重复；欠款归零；代管 4811。

**对照组（不欠他钱）**：`unpaidDebtTxs` 返回空数组，`pickDebtOffset` 选不到任何记录，
`netAmount === amount`，代管拿到整数，没有任何 `paidAt` 被标记——跟改版前行为完全一样。

**对照组（欠得比转账金额还多）**：比如欠 6000、只转 5000，逐笔累加到不超过 5000 为止，
剩下欠款还在（下次继续欠着或下次转账再抵），代管拿到的是这次转账里没用掉的余额（可能
是 0，如果刚好有一笔或几笔垫付合计正好用光这次转账）；不会出现负数，也不会拆单笔。

**撤回**（`deleteBossCashGift()`）要把抵扣一并还原：读 Firestore 文档缓存里的
`g.offsetTxIds`，把这几笔的 `fromStaff.paidAt` 删掉，恢复成未付——不还原的话，撤回了
转账、但欠款状态没跟着撤回，会变成「钱没给、欠款却凭空消失了」。

### 旧数据重算：`bossDebtRecalcScan()` / `openBossDebtRecalc()`

上面这条"转钱先抵欠款"的规则是新加的，改版之前已经转出去的钱没有做这一步——他们的
垫付欠款可能还挂着"未付"，即使代管账户里明明还有余额。「🔁 按新规则重算欠款」
（弹窗表单最下面，跟「🧹 整理旧记录」放一起）补做一次：

对每个「代管有余额（`holdingBalance(acc) > 0`）又有未结清垫付（`unpaidDebtTxs` 非空）」
的人，用**同一套** `unpaidDebtTxs` + `pickDebtOffset` 算法，把 `holdingBalance(acc)`
当作抵扣上限，选出能抵的欠款、标记 `paidAt`。

**只改 `paidAt`，不新增/删除任何交易，不碰任何账户余额**——这是刻意的设计：这纯粹是
把"记重了的欠款"改成"已经结清"，钱本身从头到尾没有再挪动一次（跟活的 `sendBossCashGift()`
不一样，那边是真的有一笔新钱在动，这里没有）。先给预览（谁、几笔、合计多少）再让用户
确认，可以重复点——标过的下次会被 `unpaidDebtTxs` 排除，天然收敛到「没有需要重算的」。
...
通用（Kuang/Seryi 互不影响）；【37】同事消费整合进真实账户的 3 条腿模型（手算 fixture：
转 5000→花 3000→真实账户净变化只有 −5000、本月支出 3000、代管余额 2000、消费明细在
真实账户里；对照组：没有代管账户的人只记一笔真支出、没有配平腿）；【38】转账先抵欠款
（欠 189 转 5000→欠款 0/代管 4811/来源账户只减 4811/支出统计不增加；对照组：不欠钱时
拿满整额、欠得比转账多时逐笔清不超额不拆单笔；撤回还原 paidAt）；【39】UI 隐藏代管
账户（对照组：真实账户照常出现在列表里）；【40】迁移工具反向搬家（真实账户不变、代管
余额不变、支出统计变多）；【41】旧数据重算欠款（预览数字、账户余额不变、只改 paidAt）。
自检脚本里的编号以文件里 `describe`/`test` 实际标注为准，这里只是帮助定位改了哪几块。
## 同事「一键归还」（2026-09-10）
...
兜底），只是它的显示出口没了。挪到 `renderBossCashGiftModal()` 弹窗表单最下面，跟
「🧹 整理旧记录」「🔁 按新规则重算欠款」放在同一行、字号缩小、不显眼——留给"万一
`forgetBossCashGift()` 的兜底摘除也没摘干净"这种极端情况手动清一下，不涉及金额，
不用二次确认。`clearBossCashGiftLog()` 内部不再调 `renderOvBossCashGift()`（卡片已经
跟这份 log 无关），改成 `toast()` 提示。

**「🧹 整理旧记录」和「🔁 按新规则重算欠款」是两个不同的工具，别混着改**：前者
（`bossCashCleanupScan()`/`openBossCashCleanup()`）搬的是"还躺在代管账户里的旧格式
消费记录"（改 `accountId` + 补配平腿）；后者（`bossDebtRecalcScan()`/`openBossDebtRecalc()`）
标记的是"该被这次转账抵掉、但当时没抵的旧欠款"（只改 `paidAt`，完全不碰交易和余额）。
两者都在同一个弹窗表单最下面，各自独立、互不依赖，详细设计见上面「代管账户」那节。
...
1. **先缩图重压再送**（`shrinkPhotoForInbox()`）：长边逐级缩 1600→1200→900→700→500、
   JPEG 质量逐级降 0.7→0.55→0.4，第一个塞得下的就用。目的是**留下看得清金额和店名的
...
自检：`check-expense-company.mjs`【44】用用户真实数字做端到端核对，五个数字逐一断言
（送出净额 4810.47、他手上剩 4586.47、用现金花掉 224、来源账户净变化 −5000、
本月支出 413.53、欠款 0），外加**两边口径一致**那条（老板端算的「他还剩」必须等于
同事端算的「手上还剩」——以后谁把任何一边改歪，这条直接红）、`own` 不走 3 条腿的对照组、
以及重复收件不让已结清垫付复活的回归。`check-staff-page.mjs` 里旧的「超支」相关断言
按新语义改写（不是删掉）。做过 false-red：把 `mergeInboxTx()` 的保留条件改成恒假，
立刻红 3 条。


### 补救：旧版重算按过之后怎么救（`openUndoMarkPaid()`，2026-09-10）

用户在**旧版**按过「🔁 按新规则重算欠款」，那一版只标 `paidAt`、**没有**把多转出去的
钱从代管余额和来源账户退回去——于是实机看到「Kuang 还剩 4776」而不是 4586.47。

**新版的重算修不了这个状态**：`bossDebtRecalcScan()` 只挑「还没结清」的垫付，已经被标成
已付的那几笔它扫不到。所以「再按一次就好了」是行不通的。

补救路径是两步，靠新增的「↩️ 撤销标记已付」：
1. **撤销**：`paidBatches()` 按 `fromStaff.paidAt` 这个时间戳分批（同一次操作标的那几笔
   时间戳一模一样，所以「一批」＝「一次操作」），撤销最近那一批。只动标记、不动金额。
   手动按过「已付」（真的另外还了现金）的批次也会列出来，**所以一定先预览再动手**，
   让用户自己认哪一批要撤，绝不自动挑。
2. **再按新版重算**：这次会连带把多转的部分从代管余额和来源账户一起退回去。

自检：`check-expense-company.mjs`【45】把他的真实处境完整重演一遍（旧版全额转 5000 ＋
垫付已被标已付 ＋ 花掉 224 → 代管 4776），断言新版重算此时确实扫不到（证明必须先撤销），
再走完两步，核对代管回到 4586.47、欠款 0、来源账户拿回 189.53、整条线净变化 −5000。
...
踩过的两个坑，都会**静默失败**（错误被 `.catch(()=>{})` 吞掉，画面上像没事）：
...
2. `openBossDebtRecalc()` 一度用 `.doc(id).update()` 改金额 → `allow update: if false`
