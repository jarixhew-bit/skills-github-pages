...
- **決策日誌（`decision_log`，2026-08-25 建）**：`analyzer.build_decision_entry()` 每個交易日
  記一筆，重點是**記下「什麼都沒做」以及為什麼**——只記成交的話，事後無法回答
  「那幾個月明明差了幾個百分點，我為什麼一直沒補」。存在 `state["decision_log"]`
  （留 400 天）與 `data-private.enc`（發到頁面的留 120 天），顯示在 index.html 的
  「🗒️ 決策日誌」區，有成交的那幾天標綠。
  改它要知道的三件事：
  (1) **台賬的日期欄位是 `trade_time`（ISO 時間戳）不是 `date`**，取前 10 位比對。
  (2) **`REBALANCE_TOLERANCE = 2.0` 跟 `build_add_suggestions` 裡 `gap >= 2` 是刻意同一個數**，
  只改一邊會出現「日誌說該補、建議區沒提」的矛盾；`tools/check-decision-log.py` 會擋住。
  (3) **欠配的標的要全部列出，不能只報最大的那個**——SGOV 與現金會占掉權重，
