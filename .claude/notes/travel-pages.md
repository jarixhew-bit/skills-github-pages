...
### 2026-08-23 第七轮：文案口吻改「专业一点」（用户明确要求，勿回退）
用户原话：「我是说用词专业一点 什么睡到12点这些」。厦门页已整页过一遍，改掉的类型：
- **不写作息玩笑／揣测客人状态**：「睡饱」「睡到中午」「看你们落地后的精神」
  「前一天玩得晚」全部改成中性说法（「当晚不安排行程」「今天是全程唯一需要早起的一天」）。
- **不写对客人的私人称呼与旁白**：「你们跟朋友出去」「你们走过很多次了」「顺手就吃了」
  「翻这一页就有」「你截图发我，我补上去」——改成陈述句。照片缺失就写
  「本店暂无可用照片：Google 地图未收录，高德照片仅在其 App 内」。
- **不写口语评价**：「最出片」→「最适合拍照」、「像换了个星球」→「与其余部分截然不同」、
  「上得了台面」→「适合宴请」、「户外基本报废」→「户外行程基本无法进行」、
  「最省事」→「最稳妥」。
- 顶部提示的「晚餐都不排（你们跟朋友出去）」整句删除；临家那条 ⚠ 三点提醒整块删除
  （用户点名移除；铺号／时间待确认仍留在 chip 上）。
英文侧同步改（同一句的 cn/en 要一起改，别只改中文）。**这条口吻标准适用于以后所有手册。**

### 2026-08-23 槟城段：不做手册（用户决定）
用户口径：槟城那几天「没啥行程 可能都是在家里休息 有需要我再通知」。
- 厦门页里那条绿色「🇲🇾 接下来的槟城段」note **已整块删除**，D4 时间线那句
  「槟城段行程另出一本」也删了。**别再自动补回去。**
- 已知的槟城段资料（留档用，页面上不放）：9月1日 MH1145 槟城 13:30 → 吉隆坡 14:35，
  转 MH762 吉隆坡 16:30 → 金边 17:25，商务舱，座位 HEW 01F／TAN 01C（两段相同），
  行李 40K。电子机票渲染图在 scratchpad `*_MH_0.png`。
- 目前唯一已知安排是**去医院**，细节未给。用户问要不要直接排进去——已回覆：
  等他给医院名称、日期时间再做，而且做成一页「就诊信息」就够（地址、地图、几点到、
  带什么），不需要整本行程手册。
