...
## 通用坑（改任何一本手册都适用，2026-09-19 新加坡那轮踩出来的）

1. **挪动行程里的地点之前，先把那天其余站点在地图上的方向过一遍。**
   只判断「这一站塞得进那天吗」会造出折返路线：把新加坡观音庙（市区）挪进 D4，
   而那天前后都在义顺（北边），实际跑法变成「北 → 市区 → 回北」，白跑约 40 公里。
   是用户问「你这样排下去那个路线呢？」才发现的，不是自己查出来的。
   判别法：读每张卡自己写的地址与车程数字，别凭印象；同向的串成一条线，别来回穿城。
2. **跨天的说明要在两天各写一遍。** 「D2 这站来不及可以改到 D4」只写在 D2 卡上，
   当天翻到 D4 的人看不到。原处写进那张卡的 `.desc`，另一天在 `.wxbox` 之后、第一张
   `.stop` 之前补一个 `.note amber`，两边都要有。
3. **顶部 `.daystrip` 的 chip 顺序必须跟区块在页面上的先后一致。** `navSetup()` 判断
   「现在停在哪一段」是 `targets.forEach(t => { if (t.offsetTop <= pos) current = t; })`，
   取的是**链接数组里最后一个**已经滚过的区块——所以 chip 排错位的那一个永远轮不到高亮。
   新加坡手册的「🍷 备用」chip 本来排在 D1 前面，可它指的 `#boss` 区块在全页最后，
   结果滑到最底下亮的是 D5，那颗 chip 从上线起就没亮过（2026-09-21 用户问
   「你一直都没发现那个备用按钮放在错的地方吗」才发现）。加 chip 时对着区块顺序排一遍。
4. **要退回前几版就整档还原，别手工逐句改回去。** 做法：先
   `git log --oneline <基准commit>..HEAD -- <手册路径>` 确认这中间没有别的改动碰过这个文件，
   再 `git checkout <基准commit> -- <手册路径>`，最后 `git diff <基准commit> -- <手册路径>`
   确认输出为空。手工回退必留孤儿句子——改一个地点动到的远不止那张卡，还有天名（`.dn`/`.dt`）、
   那几天的雨天/烟霾备案、以及别的卡片里对这站的交叉引用（「离观音庙车程 3 分钟」这类）。
...
2. **画廊图一律带 `loading="lazy" decoding="async"`**（封面 `img.bg` 除外，它在第一屏）。
   2026-09-23 出发当天补的：原本 166 张图一打开就全部下载（实测 159 个请求），
   在新加坡用漫游流量很吃数据；加上后首屏只剩约 40 个。`.gal img` 有 `aspect-ratio`
   先占好位置，所以延后加载不会让「跳到今天」落点跑掉。**新增卡片的图照这个写。**
3. D1/D2–D4 用 `.stop` 卡；**D5 用时间线** `.tl > .ev`（`.ev.hot` 强调、`.ev.fin` 收尾），
   跟 japan-trip 同一套 CSS。
4. 餐厅卡分两种：正文里的是**午餐**候选；05 区块是**老板晚餐备选**（用户 2026-08-19
   明确：措辞不要用「请」）。
...
- 机场贵宾服务是樟宜 **JetQuay CIP**（服务名 Quayside Arrival/Departure）。
  ...
  起飞前 120 分钟到。**2026-09-21 用户改口：回程那天贵宾车 10:00 出发**（此前
  2026-08-19 定的是 10:25，已作废）。故 D5 写成「10:00 出发、约 10:20 到、距起飞
  约 125 分钟，刚好达到 120 分钟，09:50 前集合完毕」——别再把「建议提早出发」写回去，
  现在时间本来就够。**退房维持 09:45**（我一度自作主张前移到 09:20 好留集合时间，
  用户 2026-09-21 说不用改、09:45 就好）。
  改这个时刻要**同时改三处**：酒店区块那条 `.note green`（中英各一）、D5 时间线的
  `.ev.hot`（`.t` 时刻＋中英说明里的到达时刻/分钟数/集合时间）；退房那格不要跟着动。
...
2026-09-19 再加**当天那块** `.airplan`（实时读数底下，预设 hidden）：这本没有逐日
行程卡，所以落点是天气区块。只有**行程期间（10/9–10/17）当天**抓到读数才露出来，
≤100 只报数字说照常、`.airact` 收着；>100 摊开「改去哪」；>150 才标红；抓不到数字
也摊开备用。跟新加坡那本同一套行为（用户：「别写死的，只在当天抓完数据写」）。
...
### 2026-08-23 照片两件事
- **海海人生的照片是用户拍的，直接放进仓库** `xiamen-trip/img/haihai-1~3.jpg`
  （门口招牌／龙虾刺身拼盘／海鲜配饭，已缩到长边 1000px、共约 280KB）。
  这是「图片一律用 Google 链接、不进仓库」那条规则的**例外**：Google 与高德都拿不到，
  用户自己给了照片。以后同样情形照办——**缩图再进仓库**，别放原图。
- **临家的 Google 照片用户说「貌似不对」**。两组关键字（含用户给的完整地址）抓回来
  指向同一个 Google 地点「临家闽南菜 4.7(18)」，照片集也几乎重叠，所以再换关键字没用。
  真正的问题是**沙盒看不到照片内容**（连不上 googleusercontent），只能凭网址交货。
  → 已建 `.github/workflows/photo-preview.yml`：喂它照片网址，它下载缩到 500px
  提交回触发分支，Claude 直接读图确认再决定用哪几张。**以后新地点的照片先过这一关**，
  别再把没看过的图放上页面。用完删掉 `.photos/preview/`。
- 临家地址已按用户给的改正：嘉禾路399号 **SM 二期购物广场黄宝石馆 4 层**
  （先前查到的「新生活广场红宝石 5 楼 C500」是错的）。

### 2026-08-23 临家照片：预览通道第一次实战
用 `photo-preview.yml` 把九张候选缩图拉回来逐张看过，证实用户说的「照片不对」：
Google 上「临家闽南菜」只有一个地点条目，**各分店照片混在一起**——先前卡上四张里
两张是海边独栋店（海景餐厅、气球壁画外观），一张是完全不像餐厅的大堂。
现用的四张（店招霓虹／上菜／大厅／店内）是看过之后挑的，卡上另加一句说明：
照片来自品牌条目、不保证就是 SM 二期这家。
**教训：抓回来的照片没看过就不要上页面。** 预览通道用完记得 `rm -rf .photos`。

### 2026-08-23 营业时间（用户手动提供，已确认，勿再标待查证）
- 临家闽南菜（SM 二期黄宝石 4 层）：**每天 10:00–14:00 · 17:00–21:00**（周一至周日相同）
- 海海人生：**11:00–14:00 · 17:00–21:00**（人均仍未知，卡上保留 ⚠）
大陆餐厅的营业时间在沙盒里查不到（点评/携程/高德全被出口代理挡），**别再花 CI 去试**，
直接问用户最快。

### 2026-08-23 上青／海海人生 的确定数据（用户从高德给的，勿再改成查来的数字）
- 上青本港海鲜：**人均 ¥233**，后江埭路29号-28，地图 `surl.amap.com/5dJPwV0jesL`
- 海海人生·上青杰哥主理：**人均 ¥1050**，后江埭路39-3号，地图 `surl.amap.com/9WYFrD078JF`
  （人均是上青的四五倍，卡上写明「最贵的一家」。两家在同一条街对面。）
两个旧短链（9aL5E7ov7Bg / 9fDXY6m1g76u）已被这两个新的取代。
用户这次的话是「再加一间 上青本港海鲜」，但清单里已经有这家、地址也对得上（跟海海人生
同街对门），所以按**更新既有卡片**处理，没有开重复卡；已在回覆里说明，若真是另一家分店
再补。

### 2026-08-23 临家照片换成用户实拍
`xiamen-trip/img/linjia-1~4.jpg`（SM 店面招牌／姜母鸭配油条／焖饭／汤），Google 那四张
与「照片来自品牌条目」的说明一并删除。至此本页两家没有 Google 照片的店（临家、海海人生）
都改用用户实拍，共 7 张、约 790KB。
另删：D1 抵达卡里「到达层 7 号门排队坐出租车／高德叫车、约 35–45 元」——有包车，用不到
（跟 2026-08-23「包车卡不要写不用叫车不用地铁」同一件事，当时漏了这处）。

### 2026-08-23 精简与三项确定（用户拍板，勿回退）
删掉：
- **所有评分标签**（乌糖 4.4 大众点评／上青 4.7 携程／植物园 4.6 携程／临家 4.7 Google／
  安达仕 4.7）——数字都来自搜索摘要或单一条目，用户认得这几家，评分没价值。
- **宴遇「周三 11:00–14:30 · 17:00–21:30」**（精确到分钟的摘要数据，风险最高）
- **上青「⚠ 旺季建议订位」**（是推的，不是查到的）
- D1 抵达卡的打车说明（见上一节）
改：
- 临家标题改成「临家闽南菜（SM 二期 · 黄宝石 4 层）」，跟地址对齐、现场好找
- **行李每件 23kg 已确定**（用户），⚠ 与「打 95557 确认」都拿掉
- **免签已确定**，「政策会调整、出发前看公告」那句拿掉
...
### 2026-08-24 安达仕酒店卡片：五张图砍到两张
用户回报「安达酒店出现破图一张」。用 `photo-preview.yml` 把卡上五张全拉回来看，结果：
- 第 2 张 `gps-proxy/ALd4DhEhvw_shjbm…` **下载失败**（CI 也拉不到）＝ 用户看到的破图。
- 第 3 张是一栋四层小楼挂「酒店」招牌、第 5 张是带 Trip.com 水印的公寓房间——**都不是安达仕**。
- 保留第 1 张（泳池＋双塔）与第 4 张（塔楼门廊夜景），这两张查证过是本店。
重抓补图失败两次，**没有第三次**：Google 该条目照片池混进了鼓浪屿民宿/古厝（八张里只有
两张是安达仕，且与现有两张重复）；高德 CI 连不上（page.goto 60s 超时）。
结论：**卡上两张就两张，不放没查证过的图**。日后要补图，优先请用户给实拍。
