...
- 机场贵宾服务是樟宜 **JetQuay CIP**（服务名 Quayside Arrival/Departure）。
  ...
  约 125 分钟，刚好达到 120 分钟，09:50 前集合完毕」，同一天的退房从 09:45 前移到
  09:20 以留出集合时间——别再把「建议提早出发」写回去，现在时间本来就够。
  改这个时刻要**同时改三处**：酒店区块那条 `.note green`（中英各一）、D5 时间线的
  `.ev.hot`（`.t` 时刻＋中英说明里的到达时刻/分钟数/集合时间）、以及退房那格 `.t`。
