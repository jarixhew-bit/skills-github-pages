...
- `sw.js`（24 行）——Service Worker，cache-first，`CACHE = 'xisui-v5'`（sw.js:1，以实际文件为准）。
...
- 版本號位置：`sw.js:1`，`const CACHE = 'xisui-v5';`（改前以實際檔案為準，勿信本筆記的舊值）。
- **硬規則：改了 `index.html`／`manifest.json` 內容就把版本號 +1（如 v5→v6），
