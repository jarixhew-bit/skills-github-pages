...
指定落檔路徑時：制度性內容放本 repo `.claude/playbook/`，一次性產物放臨時目錄
...
## 教訓紀錄
- [2026-08-01][雲端] 情境：使用者要「定時自動做某事」（月度稽核、定期檢查）。教訓：先問
  「檢測」和「判斷」能不能拆開——能拆就把檢測放 GitHub Actions（零用量），只在檢測不通過時
  才開 issue 叫 Claude 上場做需要判斷力的部分。Claude 定時任務每次觸發都要開新 session、
  載入整份 CLAUDE.md 與規則檔，為大概率沒事的檢查每月付這筆固定成本不划算。來源：制度檔
  月度體檢一開始做成 Claude Routine，使用者問「用 GitHub 的會不會更省 token」才改成 CI。
- [2026-07-11][皆是] 情境：修改本 repo 的 HTML 頁面（增刪卡片、補關閉標籤）。教訓：改完必跑
  div 開閉平衡檢查（python 逐行數 `<div`/`</div>`，出現深度 <0 或結尾 ≠0 即失敗），不平衡
  不准 commit；補「缺失的關閉標籤」時先用深度追蹤定位具體行，不憑肉眼判斷。來源：修奧林匹克
  博物館缺標籤時把 `</div>` 補錯到旬房卡片後，D07 起整段跑版，上線數日才被使用者發現。
