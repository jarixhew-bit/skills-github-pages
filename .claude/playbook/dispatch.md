...
**滲漏閘門（2026-08-19 使用者要求「能優化的都優化，token 貴」）**：上面那些門檻抓的是
「一次很大」，抓不到「一次一點、做很多次」——輪詢 CI、反覆抓同一個網站、反覆讀同一份
大檔，每一次單看都便宜，累積起來比一次讀十個檔還貴。所以再加一條硬規則：
**同一類查詢做到第三次，停手改派 subagent**，不要問自己「這次應該就好了吧」。

**先窄後寬**：動手前先挑最窄的接口，拿到結論就好，不要整包搬進對話——
`get_check_runs` 而不是 `list_workflow_jobs`；`git show ref:path | grep` 而不是讀整份檔；
`grep -n`／`awk` 抽出那幾行而不是 `Read` 全檔；MCP 工具有 `perPage`／`fields`／
`minimal_output` 就一定要帶。今天的浪費九成來自選錯接口，不是來自沒派工。
（一個例外要知道：`get_check_runs` 不能按名字過濾，PR 累積多輪 CI 後那張表會很長，
要翻頁才找得到 `all-green`；剛推過新 commit 時計數歸零，那時查最划算。）
