...
本機 settings.json 內含明文 Google Maps API key——詳情與建議只住 letter.md 第 1 件事。
...
- [2026-08-03／2026-08-08][皆是] 情境：同一份資料存在兩處（本機＋遠端／App＋帳本），使用者報
  ...
  本機刪了沒／**有沒有被同步合併回來**。取並集的合併沒有墓碑（tombstone）就一定會復活已刪的東西，
  而這層在程式碼裡看不到「刪除」二字，最容易漏；修法是刪除時落 `{id, at}` 墓碑、隨資料同步、合併剔除，
  且**每一種會被刪的東西**（08-24 只補了記錄、帳戶漏掉，刪掉的帳戶天天從雲端復活）、**每一處**移除
  路徑都要落。同理「一邊能改、另一邊只能增刪」的結構**一定會分叉**：禁止改能改的那邊（只留刪掉重
  記），或把「改」實現成「遠端刪舊＋新增新的」——「彈一句提示說那邊沒改」不算解法。
  另（2026-08-13／08-18 踩兩次）：**藏起來 ≠ 不生效**——CSS 收起來的表單欄位，存檔函式照樣讀、
  照樣送去遠端（備註欄藏了、AI 仍把商戶名填進去印上 Excel；日期欄藏了、仍送出票面日期）。
  要斷就斷在**送出那一行**（`note: null`、`date: today()`）；改完 grep 欄位 id 看還有誰在讀它。
...
- [2026-08-05／2026-08-11／2026-08-12／2026-08-17][皆是] 情境：写检查工具、或改它的等待方式，跑完报「通过」。
  ...
  **红灯同样要被验证**：会自动开 issue 的检查误报成本比漏报高，走网络的断言要先分「暂时性 vs
  确定性」（2026-08-17 live-check 红灯，实为 17 页里一页回 503 的 CDN 抖动；判别法是看同一次 run
  里别人好不好。`check-live.py` 已对 5xx/429/连线异常重试两次，404/403 照旧立刻判死）。七种踩法：
  ...
  (5) 「双击生出两条一样的记录」这类闸门自测通过、真机仍重复：双击是两个各自跑到底的事件不是竞态，加解锁
  在同一执行栈内做完等于没挡，要用 `setTimeout` 延后解锁；测法必须 `el.click(); el.click();` 连打两下。
  (6) 往既有自检档案**尾巴**追加断言等于白写：贴在「总结＋process.exit」之后照样跑、照样印 ✅，却不计入
  通过数也不影响退出码。追加前先 grep `process.exit` 插到它之前，跑完看总结那行数字有没有变大。
  (7) 断言里**写死日期＝定时炸弹**：写死月份的 fixture 到了下个月自己变红，代码一行没改（2026-09-01
  一天炸三处）。日期一律从当天算；造不出的局面（每月 1 号没有更早的同月日期）就把时钟钉死再跑那一块。
  附带：`git cat-file --batch` 的 stdin 一次写几百 KB 会死锁（管道满了两边互等），要用独立线程喂；
  占位符别写真实 NUL 字节（写 JS 转义 \u0000），否则 git 从此把文件当二进制（已由 `check-html.py` 的
  `check_control_bytes` 守着，别删）。通则：**同一类错误出现第二次就变成脚本。**
