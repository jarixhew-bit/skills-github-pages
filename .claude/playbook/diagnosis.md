...
- [2026-08-05／2026-08-11／2026-08-12／2026-08-17][皆是] 情境：写检查工具、或改它的等待方式，跑完报「通过」。
  ...
  **红灯同样要被验证**：会自动开 issue 的检查误报成本比漏报高，走网络的断言要先分「暂时性 vs
  ...
  里别人好不好。`check-live.py` 已对 5xx/429/连线异常重试两次，404/403 照旧立刻判死）。六种踩法：
  ...
  占位符别写真实 NUL 字节（写 JS 转义 \u0000），否则 git 从此把文件当二进制（踩过两次，已由
  `check-html.py` 的 `check_control_bytes` 守着，别删）。通则：**同一类错误出现第二次就变成脚本。**
