...
- [2026-08-17][雲端] 情境：`live-check.yml` 半夜紅燈、自動開了 issue，查日誌是 17 個頁面裡
  **只有一個**回 HTTP 503，其餘 16 個 200 且內容與 main 一致。教訓：**單一頁面的 5xx 幾乎一定是
  CDN 抖動，不是頁面掛了**——判別法看「其他頁面同時是好的嗎」，同一次 run 裡別人全綠就別往
  部署方向查。修法是讓檢查自己扛抖動（`check-live.py` 的 `fetch()` 對 5xx/429/連線異常等
  5 秒、20 秒各重試一次；404/403 這類明確結果不重試）。通則：**會自動開 issue 的檢查，誤報成本
  比漏報高**——任何走網路的斷言都要先分「暫時性 vs 確定性」再決定紅不紅，否則警報會被養成雜訊。
