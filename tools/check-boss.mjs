/**
 * 老板 App 自检 —— **前半**（场景一 ~ 场景十八）：权限、红点、分页、账单预览、
 * 库存、行程表单、航班、牙医。
 *
 * 共用的打桩接口与假数据在 tools/lib/boss-check-kit.mjs；后半在 check-boss-2.mjs。
 * 拆开只是为了并行跑（原本一份跑三分半，是 check-all 的瓶颈），
 * **两份加起来的断言数必须跟拆之前一样**——少一项就是漏了一条。
 *
 * 跑法：
...
// 尺寸重画了），而不是只看有没有 transform。纯 CSS 拉大位图会更糊，等于没做。
{
  ...
  // 到了 100% 减号会变灰（disabled），click 会一直等它可点——**一定要给短超时**，
  // 不然每一下都白等 Playwright 默认的 30 秒，这一段就悄悄吃掉三分钟（2026-09-05 踩过：
  // 整份自检 200 秒里有 180 秒耗在这里，还因为 catch 吞掉了看不出来）。
  for (let i = 0; i < 8; i++) { await page.click('#billZoomOut', { timeout: 1200 }).catch(() => {}); await page.waitForTimeout(120); }
