...
// ---------- 【31】给同事现金：卡片＋弹窗（2026-09-11 整套代管账户拆掉，纯备忘模式）----------
// 新模型：给现金只写 Firestore（boss_cash_gifts）＋ 本机 data.cashGifts 备忘一条，
// **不产生任何 data.transactions 记录，不建任何账户**。弹窗不再有「从哪个账户出」，
// 换成「货币」下拉（给现金不碰账户，货币只是给这笔备忘用的标签）。
{
  console.log('\n【31】给同事现金：卡片 + 弹窗 + 送出内容（纯备忘，不产生交易、不建账户）');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  ok('下拉选项来自公司报账人名册，且不含 Boss',
     (await page.evaluate(()=>Array.from(document.querySelectorAll('#boss-cash-gift-person option')).map(o=>o.value)))
       .every(v => v !== 'Boss'));
  ok('★没有「从哪个账户出」这个选择了（新模型给现金不碰任何账户）',
     (await page.locator('#boss-cash-gift-acc').count()) === 0);
  ok('★改成「货币」下拉，选项数等于内建货币表大小',
     (await page.locator('#boss-cash-gift-currency option').count()) ===
  ...
  ok('未登录时提示要先登录', (await page.textContent('#boss-cash-gift-status')||'').includes('登录'));
  ...
  // ---- 正常送出：只写 Firestore + 本机备忘，交易数一分不动 ----
  const txCountBefore = await page.evaluate(()=>data.transactions.length);
  await page.selectOption('#boss-cash-gift-person', 'Seryi');
  await page.selectOption('#boss-cash-gift-currency', 'USD');
  ...
  ok('币种是下拉选的那个', sent?.p.currency === 'USD', sent?.p.currency);
  ok('备注一起送', sent?.p.note === '给 Seryi 买菜', sent?.p.note);
  ok('带上 at 时间戳', typeof sent?.p.at === 'number', sent?.p.at);
  ok('状态行显示已送出', (await page.textContent('#boss-cash-gift-status')||'').includes('已送出'));
  ok('送出后金额栏清空',
     (await page.evaluate(()=>document.getElementById('boss-cash-gift-amount').value)) === '');

  ok('★★★data.transactions 一笔都没多（给现金不产生任何交易，这是新模型的核心）',
     (await page.evaluate(()=>data.transactions.length)) === txCountBefore,
     { before: txCountBefore, after: await page.evaluate(()=>data.transactions.length) });
  ok('★★★data.accounts 一个都没多（不再自动建代管账户）',
     (await page.evaluate(()=>data.accounts.length)) === accCountBefore,
     { before: accCountBefore, after: await page.evaluate(()=>data.accounts.length) });
  const memo = await page.evaluate(()=>data.cashGifts.find(g=>g.id==='g1'));
  ok('★本机 data.cashGifts 多了一条备忘，字段对', memo && memo.person==='Seryi' && memo.amount===600
     && memo.currency==='USD', memo);

  await page.evaluate(()=>closeModal('modal-boss-cash-gift'));
  await page.waitForTimeout(200);
  ok('首屏卡片显示 给了/花了/还剩 三个数（不是「最近转过谁多少」的显示缓存）',
     (await page.innerText('#ov-boss-cash-gift')).includes('给了') &&
     (await page.innerText('#ov-boss-cash-gift')).includes('花了') &&
     (await page.innerText('#ov-boss-cash-gift')).includes('还剩'));
  ...
  const giftCountBefore = await page.evaluate(()=>data.cashGifts.length);
  const txCountBefore2 = await page.evaluate(()=>data.transactions.length);
  ...
  ok('Firestore 送不出去时，本机 cashGifts 也没多',
     (await page.evaluate(()=>data.cashGifts.length)) === giftCountBefore);
  ok('Firestore 送不出去时，本机 transactions 也没多',
     (await page.evaluate(()=>data.transactions.length)) === txCountBefore2);

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【32】撤回一笔：删云端文档 + 移除本机备忘，没有交易要处理 ----------
{
  console.log('\n【32】撤回：删云端文档 + 移除本机备忘（没有交易，没有 paidAt 要恢复）');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  await page.evaluate(() => {
    saveBossCashKey('pass-1234');
    cloudAvailable = true; currentUser = { uid:'boss' };
    data.cashGifts = [{ id:'gX', person:'Seryi', amount:600, currency:'USD', at: Date.now() }];
    saveData();
    window.__deleted = [];
    db = { collection: () => ({
      doc: (id) => ({ delete: async () => { window.__deleted.push(id); } }),
      where: () => ({ orderBy: () => ({ limit: () => ({ get: async () => ({
        empty: false,
        docs: [{ id:'gX', data: () => ({ person:'Seryi', amount:600, currency:'USD', at: Date.now() }) }]
      }) }) }) })
    }) };
    switchTab('overview');
  });
  await page.click('#ov-boss-cash-gift');
  await page.waitForTimeout(300);
  ok('「最近转过」清单渲染出这一笔', (await page.innerText('#boss-cash-gift-recent')).includes('Seryi'));

  const txCountBefore = await page.evaluate(()=>data.transactions.length);
  await page.click('#boss-cash-gift-row-gX button');
  await page.waitForTimeout(300);
  ok('云端文档被删了', (await page.evaluate(()=>window.__deleted)).includes('gX'));
  ok('本机备忘被移除', (await page.evaluate(()=>data.cashGifts.some(g=>g.id==='gX'))) === false);
  ok('★落了墓碑（不落的话下次同步会从云端并回来）',
     await page.evaluate(()=>(data.deletedCashGiftIds||[]).some(x=>(x&&x.id)==='gX')));
  ok('★★没有交易要处理：transactions 长度全程没变', (await page.evaluate(()=>data.transactions.length)) === txCountBefore);

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【33】卡片「给了/花了/还剩」纯备忘算法 + 「他还钱了」----------
{
  console.log('\n【33】「给了/花了/还剩」纯备忘算法（累计口径，恒等式精确成立）+「他还钱了」只记负数备忘');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept('4300'));
  await ctx.route('**/*', r => r.request().url().startsWith(`http://localhost:${PORT}`)
    ? r.continue() : r.abort('failed'));
  await page.goto(URL, { waitUntil:'domcontentloaded' });
  await until(() => page.evaluate(
  ...
  await page.evaluate(() => {
    ...
    data.cashGifts = [{ id:'g1', person:'Kuang', amount:5000, currency:'USD', at: Date.now() }];
    data.transactions = [
      { id:'e1', accountId:'acc_boss', type:'expense', amount:513.97, date: today(),
        categoryId:'cat_food', description:'Kuang 花的', fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'e2', accountId:'acc_boss', type:'expense', amount:189.00, date: today(),
        categoryId:'cat_food', description:'Kuang 垫的', fromStaff:{by:'Kuang'}, paidFrom:'own' }
    ];
    saveData();
    switchTab('overview');
  });
  await page.evaluate(()=>renderOverview());
  const cardTxt = (await page.innerText('#ov-boss-cash-gift')).replace(/\s+/g,' ');
  ok('卡片显示「给了 US$5000.00　花了 US$702.97　还剩 US$4297.03」（三个数同一时间范围，恒等式精确成立）',
     cardTxt.includes('给了') && cardTxt.includes('5000.00') && cardTxt.includes('702.97') && cardTxt.includes('4297.03'),
     cardTxt);
  ...
  await page.selectOption('#boss-cash-gift-person', 'Kuang');
  await page.waitForTimeout(200);
  const summaryTxt = await page.innerText('#boss-cash-gift-person-summary');
  ok('弹窗摘要也是同一套数字', summaryTxt.includes('5000') && summaryTxt.includes('702.97') && summaryTxt.includes('4297.03'), summaryTxt);

  // ---- 他还钱了：只追加负数备忘，不碰任何交易/账户 ----
  const txBefore = await page.evaluate(()=>data.transactions.length);
  const balBefore = await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
    .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0));
  await page.evaluate(()=>repayBossCashGift());
  await page.waitForTimeout(200);
  ok('data.transactions 一笔都没多（还钱也不碰任何交易）', (await page.evaluate(()=>data.transactions.length)) === txBefore);
  ok('账户余额没变', (await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
    .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0))) === balBefore);
  const negMemo = await page.evaluate(()=>data.cashGifts.find(g=>g.amount<0));
  ok('多了一条负数备忘，金额是 prompt 填的 4300', negMemo && negMemo.amount === -4300 && negMemo.person==='Kuang', negMemo);
  const remainAfter = await page.evaluate(()=>personCashRemain('Kuang'));
  ok('还回来之后「还剩」跟着变小（5000−4300−702.97）', Math.abs(remainAfter - (5000-4300-702.97)) < 0.005, remainAfter);
...
// ---------- 【34】对每个人都通用（多人互不影响） ----------
{
  console.log('\n【34】给同事现金对每个人都通用（Kuang/Seryi 互不影响）');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  await ctx.route('**/*', r => r.request().url().startsWith(`http://localhost:${PORT}`)
  ...
  await page.evaluate(() => {
    data.cashGifts = [
      { id:'gk', person:'Kuang', amount:1000, currency:'USD', at: Date.now() },
      { id:'gs', person:'Seryi', amount:2000, currency:'HKD', at: Date.now() },
    ];
    data.transactions = [
      { id:'ek', accountId:'acc_boss', type:'expense', amount:100, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'es', accountId:'acc_yang', type:'expense', amount:300, date: today(), fromStaff:{by:'Seryi'}, paidFrom:'cash' },
    ];
    saveData();
  });
  const kRemain = await page.evaluate(()=>personCashRemain('Kuang'));
  const sRemain = await page.evaluate(()=>personCashRemain('Seryi'));
  ok('Kuang 还剩 900（1000-100），不受 Seryi 影响', Math.abs(kRemain-900)<0.005, kRemain);
  ok('Seryi 还剩 1700（2000-300），不受 Kuang 影响', Math.abs(sRemain-1700)<0.005, sRemain);
  ok('币种各自独立（Kuang USD / Seryi HKD）',
     (await page.evaluate(()=>personCashGiftCurrency('Kuang'))) === 'USD' &&
     (await page.evaluate(()=>personCashGiftCurrency('Seryi'))) === 'HKD');
  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【35】账户明细 PDF：无转账可列，「在同事手上的现金」纯备忘算 ----------
{
  console.log('\n【35】账户明细 PDF：新模型下没有 xfer，「在同事手上的现金」改用 data.cashGifts 现算');
  const ctx = await browser.newContext();
  await stubPdfLibs(ctx);
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  ...
  await until(() => page.evaluate(
    ...
    { what: 'App 启动完成' });
  await page.evaluate(() => {
    data.currentAccountId = 'acc_boss';
    data.cashGifts = [{ id:'g1', person:'Kuang', amount:5000, currency:'USD', at: Date.now() }];
    data.transactions = [
      { id:'inc1', accountId:'acc_boss', type:'income', amount:14059.71, date: today() },
      { id:'e1', accountId:'acc_boss', type:'expense', amount:513.97, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'e2', accountId:'acc_boss', type:'expense', amount:189.00, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'own' },
      { id:'e3', accountId:'acc_boss', type:'expense', amount:3854.43-513.97-189.00, date: today() },
    ];
    saveData();
  });
  await page.evaluate(()=>buildStatementPDF());
  await page.waitForTimeout(300);
  const reportHtml = await page.evaluate(()=>document.getElementById('pdf-statement').innerHTML);
  ok('★没有任何转账字样（新模型压根没有转账）', !/给.+的现金|结清|转回|配平/.test(reportHtml));
  ok('★「在同事手上的现金」行出现，金额是 4486.03（5000−513.97）',
     reportHtml.includes('在同事手上的现金') && reportHtml.includes('4,486.03'), reportHtml.includes('在同事手上的现金'));
  ok('对照组：没有代管现金活动的账户，这一行不出现', await page.evaluate(async () => {
    data.currentAccountId = 'acc_yang'; data.cashGifts = [];
    await buildStatementPDF();
    return !document.getElementById('pdf-statement').innerHTML.includes('在同事手上的现金');
  }));
  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【36】一键归还：投递箱收到 op:'repay'，只记负数备忘，不产生交易 ----------
{
  console.log('\n【36】一键归还：负数 cashGifts 备忘（不再是两条腿转账）+ 没转过现金/超额都不硬记');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  await page.evaluate(() => {
    ...
    data.cashGifts = [{ id:'g1', person:'Seryi', amount:500, currency:'USD', at: Date.now() }];
    saveData();
  });
  const seed = (id, tx, from) => page.evaluate(([id, tx, from]) =>
    window.__put(id, { k:'x', from, tx: JSON.stringify(tx) }), [id, tx, from]);
  ...
  ok('对照组：普通消费照旧记成一笔真支出', !!normalTx && normalTx.type==='expense' && !normalTx.xfer, normalTx);
  ok('★★★归还没有新增任何交易（新模型下归还只是负数备忘）',
     (await page.evaluate(()=>data.transactions.length)) === txBefore + 1 /* 只多了普通消费那一笔 */,
     { txBefore, after: await page.evaluate(()=>data.transactions.length) });
  const negMemo = await page.evaluate(()=>data.cashGifts.find(g=>g.id==='ix_repay_rs1'));
  ok('归还记成一条负数备忘，金额/人对', negMemo && negMemo.amount===-200 && negMemo.person==='Seryi', negMemo);
  ok('还剩变成 300（500-200）', Math.abs((await page.evaluate(()=>personCashRemain('Seryi')))-300)<0.005);

  // ---- 幂等：同一份文档没删掉又被收一次 ----
  await seed('r1b', { op:'repay', srcId:'rs1', amount:200, date:'2026-09-10' }, 'Seryi');
  await fetchNow(); await page.waitForTimeout(300);
  ok('同一笔归还重复收到不会记第二次（幂等）',
     (await page.evaluate(()=>data.cashGifts.filter(g=>g.id==='ix_repay_rs1').length)) === 1);

  // ---- 没转过现金的人送归还：不许硬记 ----
  const giftCountBefore = await page.evaluate(()=>data.cashGifts.length);
  await seed('r2', { op:'repay', srcId:'rs2', amount:50, date:'2026-09-10' }, '从来没转过钱的人');
  await fetchNow(); await page.waitForTimeout(300);
  ok('没转过现金的人送归还，不硬记', (await page.evaluate(()=>data.cashGifts.length)) === giftCountBefore);
  ok('这份文档留着，等老板手动处理', await page.evaluate(()=>window.__box.docs.some(d=>d.id==='r2')));

  // ---- 还的比手上的多：不许硬记 ----
  const remainNow = await page.evaluate(()=>personCashRemain('Seryi'));   // 此时剩 300
  const beforeLen3 = await page.evaluate(()=>data.cashGifts.length);
  await seed('rover', { op:'repay', srcId:'rsover', amount: remainNow + 1, date:'2026-09-10' }, 'Seryi');
  await fetchNow(); await page.waitForTimeout(300);
  ok('归还金额超过手上现金时不入账', (await page.evaluate(()=>data.cashGifts.length)) === beforeLen3);
  ok('提示说明是超额', (await page.textContent('#toast')||'').includes('超过'));

  // ---- 对照：刚好等于余额的一笔要放行 ----
  await seed('rexact', { op:'repay', srcId:'rsexact', amount: remainNow, date:'2026-09-10' }, 'Seryi');
  await fetchNow(); await page.waitForTimeout(300);
  ok('对照组：刚好等于余额的「全部归还」照常入账', await page.evaluate(()=>data.cashGifts.some(g=>g.id==='ix_repay_rsexact')));
  ok('全部归还后还剩归零', Math.abs(await page.evaluate(()=>personCashRemain('Seryi'))) < 0.005);

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【37】同事报回来的消费一律记成一笔普通真支出（不再有配平腿/账户路由）----------
{
  console.log('\n【37】fetchInbox()：消费一律记一笔真支出（对照组：有没有代管现金记录都一样，paidFrom 原样存进 tx）');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  await page.evaluate(() => {
    ...
    currentUser = { uid: 'boss' };
    db = { collection: () => ({ limit: () => ({ get: async () => ({ docs: window.__box.docs }) }) }) };
    // Kuang 曾经收过现金（有 cashGifts 记录）；Yang 从来没收过——两个人都要走同一条入账路径
    data.cashGifts = [{ id:'g1', person:'Kuang', amount:5000, currency:'USD', at: Date.now() }];
    saveData();
  });
  const seed = (id, tx, from) => page.evaluate(([id, tx, from]) =>
  ...
  await seed('k1', { srcId:'k1', date:'2026-09-10', amount:100, type:'expense',
                     categoryId:'cat_food', description:'Kuang 花的（现金付）', paidFrom:'cash' }, 'Kuang');
  await seed('y1', { srcId:'y1', date:'2026-09-10', amount:200, type:'expense',
                     categoryId:'cat_food', description:'Yang 花的（自己垫）', paidFrom:'own' }, 'Yang');
  await fetchNow(); await page.waitForTimeout(300);

  const kTx = await page.evaluate(()=>data.transactions.find(t=>t.id==='ix_k1'));
  const yTx = await page.evaluate(()=>data.transactions.find(t=>t.id==='ix_y1'));
  ok('★Kuang（有代管现金记录）的消费只记一笔，没有配平腿', !!kTx && !kTx.xfer, kTx);
  ok('★Yang（没有代管现金记录）的消费也只记一笔——两人走同一条路', !!yTx && !yTx.xfer, yTx);
  ok('两笔都落进「收件设置」那个账户', kTx.accountId === (await page.evaluate(()=>getInboxAccountId()))
     && yTx.accountId === (await page.evaluate(()=>getInboxAccountId())));
  ok('★没有任何配平腿产生（总共只多了这 2 笔，不是 6 笔）',
     (await page.evaluate(()=>data.transactions.filter(t=>t.fromStaff).length)) === 2);
  ok('paidFrom 原样存进 tx（cash/own），供「该付同事多少」判断', kTx.paidFrom==='cash' && yTx.paidFrom==='own', {kTx,yTx});
  ok('★★★整批收件全程没有任何 xfer 交易', !(await page.evaluate(()=>data.transactions.some(t=>t.xfer))));

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【38】该付同事多少：原样保留，只是不再跟转账绑在一起（对照组）----------
{
  console.log('\n【38】isUnpaidStaffAdvance/inboxOwedByPerson/inboxMarkPaid 保留：paidFrom=cash 不欠、own 欠、已付不欠');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  await page.evaluate(() => {
    data.transactions = [
      { id:'a1', accountId:'acc_boss', type:'expense', amount:189, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'own' },
      { id:'a2', accountId:'acc_boss', type:'expense', amount:100, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'a3', accountId:'acc_boss', type:'expense', amount:50, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'own', fromStaffPaidAt: 1 },
    ];
    data.transactions[2].fromStaff.paidAt = Date.now();   // 已经标付清的
    saveData();
  });
  const owed = await page.evaluate(()=>inboxOwedByPerson());
  ok('只有 paidFrom=own 且没标已付的那笔算欠款（189，不含 cash 的 100，也不含已标付的 50）',
     owed.length===1 && Math.abs(owed[0].total-189)<0.005, owed);
  // 给他转 5000（新模型：不再自动结清）
  await page.evaluate(() => {
    saveBossCashKey('pass-1234');
    cloudAvailable = true; currentUser = { uid:'boss' };
    db = { collection: () => ({ add: async () => ({ id:'gz' }) }) };
    document.getElementById('boss-cash-key-input') && (document.getElementById('boss-cash-key-input').value='pass-1234');
  });
  await page.evaluate(()=>switchTab('overview'));
  await page.click('#ov-boss-cash-gift');
  await page.waitForTimeout(200);
  ...
  await page.evaluate(()=>sendBossCashGift());
  await page.waitForTimeout(300);
  const owedAfter = await page.evaluate(()=>inboxOwedByPerson());
  ok('★转账之后欠款不变（189）——转账不再自动结清任何东西', owedAfter.length===1 && Math.abs(owedAfter[0].total-189)<0.005, owedAfter);
  await page.evaluate(()=>closeModal('modal-boss-cash-gift'));
  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【39】用户真实数字端到端（2026-09-11 拆代管账户之后的最终形态）----------
{
  console.log('\n【39】真实数字端到端：14059.71 注资 − 3854.43 开销 → 账户 10205.28；卡片 给了5000 花了702.97 还剩4297.03');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  page.on('dialog', d => d.accept());
  ...
  await page.evaluate(() => {
    data.currentAccountId = 'acc_boss';
    data.transactions = [
      { id:'inc', accountId:'acc_boss', type:'income', amount:14059.71, date: today() },
      // Kuang 用现金花的（513.97）＋ 自己垫的两笔 Boss 的饭钱（21+168=189，paidFrom:own）
      { id:'e1', accountId:'acc_boss', type:'expense', amount:513.97, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'e2', accountId:'acc_boss', type:'expense', amount:21.00, date: '2026-09-09', fromStaff:{by:'Kuang'}, paidFrom:'own' },
      { id:'e3', accountId:'acc_boss', type:'expense', amount:168.00, date: '2026-09-09', fromStaff:{by:'Kuang'}, paidFrom:'own' },
      // 其余不相关的开销，凑满 3854.43
      { id:'e4', accountId:'acc_boss', type:'expense', amount: Math.round((3854.43-513.97-21-168)*100)/100, date: today() },
    ];
    saveData();
  });
  const balBefore = await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
    .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0));
  ok('转账前账户余额 = 10205.28（14059.71−3854.43）', Math.abs(balBefore-10205.28)<0.005, balBefore);
  ...
  await page.evaluate(() => {
    ...
    cloudAvailable = true; currentUser = { uid:'boss' };
    window.__gifts = [];
    db = { collection: () => ({ add: async (p) => { window.__gifts.push(p); return {id:'gReal'}; } }) };
    switchTab('overview');
  });
  await page.click('#ov-boss-cash-gift');
  await page.waitForTimeout(200);
  ...
  const balAfter = await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
    .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0));
  ok('★★★转账后账户余额不变，仍是 10205.28（转账不记账）', Math.abs(balAfter-10205.28)<0.005, balAfter);
  ok('★★★转账前后 data.transactions.length 完全相同', (await page.evaluate(()=>data.transactions.length)) === txCountBefore,
     { before: txCountBefore, after: await page.evaluate(()=>data.transactions.length) });
  ok('★★★全部交易里没有任何一条打了 xfer', !(await page.evaluate(()=>data.transactions.some(t=>t.xfer))));

  const given = await page.evaluate(()=>personCashGiftsGiven('Kuang'));
  const spent = await page.evaluate(()=>personSpendTotal('Kuang'));
  const remain = await page.evaluate(()=>personCashRemain('Kuang'));
  ok('给了 = 5000.00', Math.abs(given-5000)<0.005, given);
  ok('花了 = 702.97（513.97+189.00，累计）', Math.abs(spent-702.97)<0.005, spent);
  ok('还剩 = 4297.03（给了−花了）', Math.abs(remain-4297.03)<0.005, remain);
  const cardTxt = (await page.innerText('#ov-boss-cash-gift')).replace(/\s+/g,' ');
  ok('卡片文字跟上面三个数一致', cardTxt.includes('5000.00') && cardTxt.includes('702.97') && cardTxt.includes('4297.03'), cardTxt);

  // ---- 对照组：还没拿到现金就先垫钱的人，「该付同事多少」照样看得到 ----
  const owed = await page.evaluate(()=>inboxOwedByPerson());
  ok('对照组：Kuang 还欠 189.00（转账没有顺带结清）', owed.some(r=>r.who==='Kuang' && Math.abs(r.total-189)<0.005), owed);

  // ---- 撤回转账：备忘消失、卡片回到转账前、交易一笔都没动 ----
  await page.evaluate(() => {
    db = { collection: () => ({
      doc: () => ({ delete: async () => {} }),
      where: () => ({ orderBy: () => ({ limit: () => ({ get: async () => ({
        empty: false, docs: [{ id:'gReal', data: () => ({ person:'Kuang', amount:5000, currency:'USD', at: Date.now() }) }]
      }) }) }) })
    }) };
  });
  await page.evaluate(()=>renderBossCashGiftModal());
  await page.waitForTimeout(300);
  const txBeforeUndo = await page.evaluate(()=>data.transactions.length);
  await page.click('#boss-cash-gift-row-gReal button');
  await page.waitForTimeout(300);
  ok('撤回后备忘消失', (await page.evaluate(()=>data.cashGifts.some(g=>g.id==='gReal'))) === false);
  ok('撤回后「还剩」回到转账前（0，因为给了归零）', Math.abs(await page.evaluate(()=>personCashGiftsGiven('Kuang'))) < 0.005);
  ok('撤回全程交易一笔都没动', (await page.evaluate(()=>data.transactions.length)) === txBeforeUndo);

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【40】一键转换旧资料：先预览、用户确认才动手（对照组：预览数字与手算一致）----------
{
  console.log('\n【40】一键转换旧资料：预览数字对得上手算 + 转换后没有任何 xfer 残留 + 每条移除的都有墓碑');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  const dialogs = [];
  ...
  // 造一份旧格式资料：转账 5000（最终格式，两条腿全额）＋ 结清 189（只在代管账户那条）
  // ＋ 3 条腿消费（花 513.97）＋ 一笔更旧的手动「他还钱了」（100，没有任何 id 串起来）
  await page.evaluate(() => {
    const now = Date.now();
    const hold = { id:'acc_hold_kuang', name:'在 Kuang 手上', currency:'USD', color:'#f59e0b', isHolding:true, createdAt: now };
    data.accounts.push(hold);
    data.transactions = [
      { id:'inc', accountId:'acc_boss', type:'income', amount:14059.71, date: today() },
      { id:'other', accountId:'acc_boss', type:'expense', amount:3854.43-513.97-189-21-168, date: today() },
      // 转账两条腿（全额）
      { id:'g_src', accountId:'acc_boss', type:'expense', amount:5000, date: today(),
        categoryId:'cat_cash_gift', description:'给 Kuang 的现金', updatedAt: now, xfer:true, giftId:'giftA' },
      { id:'g_hold', accountId: hold.id, type:'income', amount:5000, date: today(),
        categoryId:'cat_cash_gift_in', description:'从「Boss」转入', updatedAt: now, xfer:true, giftId:'giftA' },
      // 结清腿（只在代管账户那条，189）
      { id:'g_settle', accountId: hold.id, type:'expense', amount:189, date: today(),
        categoryId:'cat_cash_gift', description:'结清 Kuang 垫付的 2 笔（配平）',
        updatedAt: now, xfer:true, giftId:'giftA', settleAdvanceIds:['d1','d2'] },
      // 垫付本身（已标 paidAt，代表已结清）
      { id:'d1', accountId:'acc_boss', type:'expense', amount:21, date:'2026-09-09',
        fromStaff:{by:'Kuang', paidAt: now}, categoryId:'cat_food', description:'Boss lunch' },
      { id:'d2', accountId:'acc_boss', type:'expense', amount:168, date:'2026-09-09',
        fromStaff:{by:'Kuang', paidAt: now}, categoryId:'cat_food', description:'Boss dinner' },
      // 3 条腿消费（花 513.97）
      { id:'spend1', accountId:'acc_boss', type:'expense', amount:513.97, date: today(),
        categoryId:'cat_food', description:'Kuang 花的', fromStaff:{by:'Kuang'}, staffSpendId:'spend1' },
      { id:'spend1_leg2', accountId:'acc_boss', type:'income', amount:513.97, date: today(),
        categoryId:'cat_cash_gift_in', description:'Kuang 用代管现金支付（配平）', updatedAt: now, xfer:true, staffSpendId:'spend1' },
      { id:'spend1_leg3', accountId: hold.id, type:'expense', amount:513.97, date: today(),
        categoryId:'cat_cash_gift', description:'Kuang 用代管现金支付（配平）', updatedAt: now, xfer:true, staffSpendId:'spend1' },
      // 更旧的手动「他还钱了」（100，没有 giftId/repayId/staffSpendId，只有固定模板描述）
      { id:'oldrepay_hold', accountId: hold.id, type:'expense', amount:100, date: today(),
        categoryId:'cat_cash_gift', description:'Kuang 还回来的现金（转回「Boss」）', updatedAt: now, xfer:true },
      { id:'oldrepay_real', accountId:'acc_boss', type:'income', amount:100, date: today(),
        categoryId:'cat_cash_gift_in', description:'Kuang 还回来的现金（从代管账户转回）', updatedAt: now, xfer:true },
    ];
    saveData();
  });
  ...
  const balBefore = await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
    .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0));
  // 手算改前余额：14059.71 + 100(旧还钱) − other − 5000(转账) − 21 − 168 − 0(spend1 不在这个账户)
  // 用代码里同一套公式核对，不重复推导，只断言"跑代码得到的余额"跟"手算预期"一致
  ok('改前 acc_boss 余额跟手算一致', Math.abs(balBefore - (14059.71 + 100 - (3854.43-513.97-189-21-168) - 5000 - 21 - 168)) < 0.005, balBefore);

  await page.evaluate(()=>{ document.getElementById('toast').textContent=''; switchTab('settings'); });
  await page.evaluate(()=>convertHoldingModelToMemo());
  await page.waitForTimeout(300);
  const preview = dialogs[dialogs.length-1] || '';
  ok('预览讲清楚账户余额会从 X 变成 Y', /会从.+变成/.test(preview), preview);
  ok('预览讲清楚卡片会变成 给了/花了/还剩', preview.includes('给了') && preview.includes('花了') && preview.includes('还剩'), preview);
  ok('预览数字：Kuang 给了 5000.00 花了 702.97 还剩 4297.03', preview.includes('5000.00') && preview.includes('702.97') && preview.includes('4297.03'), preview);

  ok('★转换后账户余额 = 10205.28（14059.71−3854.43）',
     Math.abs((await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
       .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0))) - 10205.28) < 0.005,
     await page.evaluate(()=>data.transactions.filter(t=>t.accountId==='acc_boss')
       .reduce((s,t)=>t.type==='income'?s+t.amount:s-t.amount,0)));
  ok('★没有任何 xfer 残留', !(await page.evaluate(()=>data.transactions.some(t=>t.xfer))));
  ok('★代管账户被删掉了', (await page.evaluate(()=>data.accounts.some(a=>a.isHolding))) === false);
  const given40 = await page.evaluate(()=>personCashGiftsGiven('Kuang'));
  const spent40 = await page.evaluate(()=>personSpendTotal('Kuang'));
  ok('转换后 给了=5000 花了=702.97', Math.abs(given40-5000)<0.005 && Math.abs(spent40-702.97)<0.005, {given40, spent40});
  ok('★每一条移除的记录都落了墓碑', await page.evaluate(()=>{
    const dead = new Set((data.deletedTxIds||[]).map(x=>x.id));
    return ['g_src','g_hold','g_settle','spend1_leg2','spend1_leg3','oldrepay_hold','oldrepay_real']
      .every(id => dead.has(id));
  }));
  ok('真支出腿（spend1）留着，补了 paidFrom:cash，清掉了 staffSpendId', await page.evaluate(()=>{
    const t = data.transactions.find(x=>x.id==='spend1');
    return !!t && t.paidFrom==='cash' && !t.staffSpendId;
  }));
  ok('结清引用的垫付 paidAt 原样不动（既成事实，不因为换记账手法而反悔）', await page.evaluate(()=>{
    const d1 = data.transactions.find(x=>x.id==='d1');
    return !!d1 && !!d1.fromStaff.paidAt;
  }));
  ...
  await page.evaluate(()=>convertHoldingModelToMemo());
  await page.waitForTimeout(200);
  ok('可重复点：第二次提示没有需要转换的', (await page.textContent('#toast')||'').includes('没有旧的代管账户资料'));

  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}

// ---------- 【41】两边口径一致：老板端「还剩」＝ 同事端「手上还剩」 ----------
// 同事端口径不变（收到−花掉，见 tools/build-staff-page.py 的 bossCashLeft()/renderBossCash()），
// 跟老板端拆不拆代管账户无关——这条只是把两边的公式重放一遍，钉死"以后谁改歪都会红"。
{
  console.log('\n【41】两边口径一致：老板端 personCashRemain() ＝ 同事端「手上还剩」（got−月度花掉）');
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e=>errs.push(e.message));
  await ctx.route('**/*', r => r.request().url().startsWith(`http://localhost:${PORT}`)
  ...
  await page.evaluate(() => {
    ...
    data.transactions = [
      { id:'e1', accountId:'acc_boss', type:'expense', amount:513.97, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'cash' },
      { id:'e2', accountId:'acc_boss', type:'expense', amount:189, date: today(), fromStaff:{by:'Kuang'}, paidFrom:'own' },
    ];
    saveData();
  });
  const bossRemain = await page.evaluate(()=>personCashRemain('Kuang'));
  // 同事端公式重放：got（5000）− 本月花掉合计（cash+own 都算，见 bossCashMonthSpend()）
  const staffRemain = await page.evaluate(()=>{
    const got = 5000;
    const monthSpend = data.transactions.filter(t=>t.type==='expense' && t.fromStaff && t.fromStaff.by==='Kuang')
      .reduce((s,t)=>s+t.amount,0);
    return Math.round((got - monthSpend) * 100) / 100;
  });
  ok('两边口径一致（这次全部活动都在本月，累计口径与月度口径巧合式一致）',
     Math.abs(bossRemain - staffRemain) < 0.005, { bossRemain, staffRemain });
  ok('无 JS 报错', errs.length===0, errs);
  await ctx.close();
}
