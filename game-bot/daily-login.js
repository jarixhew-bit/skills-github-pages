...
var CONFIG = {
    ...
    // 板块越多要给得越宽：五个板块跑完大约 3~6 分钟，15 分钟留了余量。
    "总超时分钟": 15,
    ...
    //
    // 下面按「板块」分组：签到 / 挂机收益 / 体力 / 邮件 / 日常任务。
    //
    // ★ 只裁你想要的那几张模板图就行。没裁的图，那一步会立刻跳过（不会空等），
    //   所以你可以先只弄签到，跑顺了再一块一块加，不必一次裁十几张。
    //
    // ★ 每个板块前面都有一步「回主界面」。别删它。没有它的话，上一个板块万一
    //   没退干净（邮件界面还开着），后面所有板块都会在错的界面上找图，全军覆没。
    "步骤": [
        ...
        // ───── 每日签到（放最前面：它是唯一「必须」的板块，
        //       万一失败中止，也不会连累后面几块）─────
        {
            "名字": "签到 · 回主界面",
            "类型": "回主界面",
            "图": "main.png",
            "退出框取消图": "exit_cancel.png",
            "必须": false
        },
        {
            "名字": "签到 · 点入口",
...
// 找一次的容错版：模板图没裁、读不出来时返回 null 而不是抛错。
// 「回主界面」这类保底动作要用它——不能因为某张图没准备好就整个跑不动。
function 试找(模板名) {
    try {
        return 找一次(模板名);
    } catch (e) {
...
function 跑一步(步) {
    ...
    if (类型 === "回主界面") {
        // 每个板块开跑前都要先归位。理由：上一个板块万一没退干净（邮件界面还开着），
        // 后面所有步骤都会在错误的界面上找图，一路失败到底。有了这一步，
        // 某个板块失败只影响它自己，不会把后面的全带塌。
        var 主图 = 步["图"] || "main.png";
        var 上限 = 步["最多按几次"] || 6;
        记("回主界面（最多按 " + 上限 + " 次返回）：" + 名字);
        for (var b = 0; b < 上限; b++) {
            if (超时了()) return false;
            if (试找(主图)) {
                记("✓ 已在主界面");
